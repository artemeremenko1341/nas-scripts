...
Важно: в отличие от предыдущей версии на Rocky/VPS:
  ...
  - Медиа кладутся ВНУТРЬ часового блока как `- HH:MM <img src=...>`, а не
    точечным заголовком `## HH:MM`
  - Хранение целей перенесено: уже лежим на Synology, пути локальные

Формат вставки внутри блока:
  ## 14:00 - 15:00

  - 14:47 <img src="file:///Z:/home/Photos/.../20260417_144700.jpg">
  - 14:23 <video src="file:///Z:/home/Photos/.../20260417_142300.mp4" controls></video>
...
def parse_date_time(filename: str) -> tuple[str | None, str | None]:
    ...
    for pat in _PATTERNS:
        ...
        if not m:
            continue
        gd = m.groupdict()
        d_str = f"{gd['y']}-{gd['mo']}-{gd['d']}"
        t_str = f"{gd['h']}:{gd['mi']}" if gd.get("h") else None
        return d_str, t_str
    return None, None
...
def collect_files_for_day(target_date: date, logger: logging.Logger) -> list[dict]:
    ...
    def scan_dir(src_path: Path, src_name: str) -> None:
        ...
        for p in entries:
            ...
            if fdate != target_str:
                continue
            # Для файлов без времени в имени — пытаемся взять из mtime
            if ftime is None:
                try:
                    mtime = datetime.fromtimestamp(p.stat().st_mtime)
                    # но только если дата mtime совпадает с целевой
...
def media_bullet(time_str: str, is_photo: bool, fname: str, is_obs: bool) -> str:
    """- HH:MM <img src=...> | <video src=... controls></video>"""
